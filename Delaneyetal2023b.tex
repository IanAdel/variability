...
Here we look at three possibilities, namely, rivers, steady-state
R-channels, and pipe-flow (i.e. R-channels which do not have time to
adjust their size to discharge conditions).

Basics of hydraulics
\begin{equation}
  \Psi \propto f\frac{v^2}{D}
\end{equation}
or
\begin{equation}
  \Psi \propto f\frac{Q^2}{D^5}.
\end{equation}
Then
\begin{equation}
  \tau \propto v^2 = (Q/S)^2.
...
For all flow types we assume that the discharge $Q$ is given.
A flow in a wide river has $\Psi$ constant (given by topography), $w \gg h$, and we assume the width can be approximated by a relation $w \propto Q^\alpha$ with $\alpha\in [0,1]$ and typically $\alpha \approx 1/2$.  A R-channel flow has $\Psi$ constant (approximated by the Shreve potential) and $S$ adjusts such that it is in steady state.  Pipe flow has $S$ fixed and $\Psi$ adjusts.

Shear stresses are given by
\begin{align}
  \text{wide river} \quad\quad &  \tau \propto \left(\frac{Q}{w}\right)^{2/3} \Psi^{2/3}\\
  \text{R-channel} \quad\quad & \tau =  \\
  \text{pipe} \quad\quad &  \tau \propto v^2 \propto\\
\end{align}
...
Basics on sediment transport
\begin{align}
  \text{MPM} \quad\quad & Q_s \propto w \tau^{3/2}\\
  \text{EH}  \quad\quad& Q_s \propto w \tau^{5/2}\\
  \text{Bagnold}  \quad\quad & Q_s \propto w (\Psi Q)^{3/2}
\end{align}
for conditions well above transport onset threshold.

% \begin{tabular}{lll}
%  & Shear stress & Stream power\\[0pt]
%  & \(\tau \propto\) & \(\Omega \propto\)\\[0pt]
% \hline
% River & \(\big(\tfrac{Q}{w}\big)^{2/3} \Psi^{2/3}\) & \(Q \Psi\)\\[3pt]
% R-channel & \(Q^{2/5} \, \Psi^{4/5}\) & \(Q \Psi\)\\
% Pipe & \(Q^2\) & \(Q^3\)\\
% \end{tabular}

\noindent
\begin{tabular}{llllll}
Kind & Fixed & Determined via D-W & Additional relations & Shear stress & Stream p.\\[0pt]
 &  &  &  & \(\tau \propto\) & \(\Omega \propto\)\\[0pt]
\hline
