...
The river is assumed to have a width $W$ much greater than its depth $H$, such that $D_h\approx 4H$, and to have a constant head gradient ($\Psi$) given by the topography.  Further, we assume that its width can be approximated by a relation $w \propto Q^\alpha$ with $\alpha\in [0,1]$; typically $\alpha \approx 1/2$, $\alpha=0$ or $\alpha=1$ correspond a river of constant width or depth, respectively.
%
...
\mauro{Bagnold gives pretty crazy results. Check again!}

Table~\ref{tab:Qs2} shows that the proxy $W\tau$ as well as the MPM and EH sediment transport formulas give roughly a similar scaling of $Q_s$ on $\Psi$ or $S$ and on $Q$.  The total transport formula EH giving a stronger dependence on all variables as should be expected duto to the larger exponent on $\tau$ of 2.5 versus 1.5 for the others.  The exponent for the width scaling $\alpha$ only impacts the EH relation, with its value almost certainly around or bigger than 0.5 for a braided proglacial stream.
%
The sediment discharge in the steady state R-channel scales very similarly to the river case, and almost identically for the $\alpha=0.5$ case.  However, note that the head gradients $\Psi$ are likely much higher for comparable $Q$ in a glacial setting than a river setting, as is well described in \citeA{alley1997}, and thus transport rates may still be much higher in a steady state R-channel.
%
Furthermore, R-channels will rarely operate in steady state as variations in discharge, in particular on the diurnal timescale or the timescale of severe rain or melting events, are too fast for such a channel to reach a steady state.  In such cases they operate more like a pipe of fixed cross-section.  Table~\ref{tab:Qs2} shows that then the sediment transport scales much more severely with discharge, with the exponent of $Q$ being between 2 and 5, than in the other two regimes when that exponent is at most 1.3.  Thus fluctuations of discharge on short timescales (on the order of a day) have the potential to cause very high sediment transport conditions.
...
R-ch. & \(\Psi\) & \(S\, \propto f^{2/5}\, Q^{4/5} \, \Psi^{-2/5}\) & \(D_h\propto S^{1/2}\) & \(f^{1/5} Q^{2/5} \, \Psi^{4/5}\) & \(Q \Psi\)\\
Pipe & \(S\) & \(\Psi \propto f \, Q^2\, S^{-5/2}\) & \(D_h\propto S^{1/2}\) & \(f Q^2 S^{-2}\) & \(f Q^3 S^{-5/2}\)\\
\end{tabular}
\end{table}
...
\begin{tabular}{lllll}
 ...
 & \(W\, \tau\) & \(Q_s \propto W\, \tau^{3/2}\) & \(Q_s \propto W\, \tau^{5/2}\) & \(Q_s \propto W\, \Omega^{3/2} H^{-2/3}\)\\
\hline
River & \(f^{1/3}\, Q^{2/3+\alpha/3}\,  \Psi^{2/3}\) & \(f^{1/2}\, Q \, \Psi\) & \(f^{5/6}\, Q^{5/3 - 2\alpha/3} \, \Psi^{5/3}\) & \(f^{-2/9}\, Q^{13\alpha/9 - 19/18} \, \Psi^{31/18}\)\\
R-ch. & \(f^{2/5}\, Q^{4/5} \, \Psi^{3/5}\) & \(f^{1/2}\, Q \, \Psi\) & \(f^{7/10}\, Q^{7/5}\, \Psi^{9/5}\) & \(f^{1/15}\, Q^{49/30}\, \Psi^{43/30}\)\\
Pipe & \(f \, Q^2 \, S^{-1}\) & \(f^{3/2}\, Q^3 \, S^{-5/2}\) & \(f^{5/2}\, Q^5\, S^{-9/2}\) & \(f^{3/2} \, Q^{9/2} \, S^{-43/12}\)\\
\end{tabular}
\end{table}
...
River (\(\alpha=0.5\)) & \(f^{0.7}\, Q^{0.8}\,  \Psi^{0.7}\) & \(f^{0.5}\, Q \,\,\,\,\Psi\) & \(f^{0.8}\, Q^{1.3} \, \Psi^{1.7}\) & \(f^{-0.2}\, Q^{-0.3} \, \Psi^{1.7}\)\\
River (\(\alpha=1\)) & \(f^{0.7}\, Q^{1.0}\,  \Psi^{0.7}\) & \(f^{0.5}\, Q \,\,\,\, \Psi\) & \(f^{0.8}\, Q^{0.7} \, \Psi^{1.7}\) & \(f^{-0.2}\, Q^{0.4} \, \Psi^{1.7}\)\\[3pt]
R-ch. & \(f^{0.4}\, Q^{0.8} \, \Psi^{0.6}\) & \(f^{0.5}\, Q \,\,\,\, \Psi\) & \(f^{0.7}\, Q^{1.4}\, \Psi^{1.8}\) & \(f^{0.07}\, Q^{1.6}\, \Psi^{1.4}\)\\
Pipe & \(f \quad\, Q^{2\phantom{.0}} \, S^{-1}\) & \(f^{1.5}\, Q^3 \, S^{-2.5}\) & \(f^{2.5}\, Q^{5\phantom{.0}}\, S^{-4.5}\) & \(f^{1.5} \, Q^{4.5} \, S^{-3.6}\)\\
\end{tabular}
\end{table}
