...
\begin{abstract}
  ...
  Here, we present formulations of sediment transport capacity in different channel types and apply subglacial and subaerial hydraulics models to hydrographs from an Alpine glacier and a catchment of the Greenland Ice Sheet. 
  ...
  The implications of these findings help to evaluate sediment discharge from glaciers with different hydro-climatic forcings.
...
Many of these studies discuss the disparity between water discharge and sediment export, especially given the role of sediment availability or production.
Yet, a variable relationship between water discharge and sediment export could be expected from glaciers with evolving conduit size in a transport-limited regime.
...
Water discharge was measured roughly $2$\unit{km} downstream from the terminus ($67^\circ\,03'\,5''$ N, $50^\circ\,12'\,59''$ W), at a $5$\,\unit{min} time interval from May 28, 2012 to August 8, 2012 \citep[][; Figure~\ref{fig:model_outs}\,b]{tedstone2013}.
...
We note that the subglacial results apply to pressurized subglacial channels, not depressurized ones that can occur underneath glacier termini \citep{perolo2018}.
...
Sediment transport in pipe-flow scales much more severely with discharge (Table~\ref{tab:Qs}); the exponent on $Q$ being between $3$ and $5$, compared to the other two channel types when that exponent is at most $\frac{5}{3}$.
...
The greater sediment transport capacity in the subglacial channels implies that sediment grain size underneath subglacial channels is likely larger than subaerial counterparts, as smaller clasts would be evacuated.
Note, however, that the mean water velocities are similar in both channel types.
...
Covariance also occurs for steady-state R-channels (Section \ref{sect:timing}; Figure \ref{fig:chan_types} d).
...
More generally, the same process could also result in sediment discharge's strong dependence on sediment production from glacial sliding, which is dependent on basal shear stress of the glacier \citep{herman2015,koppes2015}.
...
Indeed, the more decoupled and variable relationship exists between model outputs and water discharge in \alpine{} results from the relatively larger discharge variations on sub-daily to weekly timescales (Figure~\ref{fig:Qw_vari}).
...
The manuscript's first objective is to establish the conditions where water discharge covaries with subglacial sediment transport capacity.
...
Water discharge variations in potentially  representative Greenland Ice Sheet and Alpine catchments are substantial enough to cause this inconsistent relationship, even when water discharge records are smoothed.
...
Few observations of the subglacial environment hinders our ability to quantify processes such as
...
New methods are being developed to quantify these processes \citep{jenkin2023},  better informing the response of sediment transport capacity to water discharge forcing in subglacial environments.
...
\competinginterests{One or more of the (co-) authors is a member of The Cryosphere editorial board and is not involved in the review process.} 
