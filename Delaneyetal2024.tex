...
\begin{abstract}
  ...
  Using numerical models applied to two observed hydrographs and sediment transport formulations, we evaluate the differences in sediment transport capacity variability between subaerial and subglacial channels.
  The numerical experiments confirm that subglacial sediment transport is highly non-linear with respect to water discharge.
  ...
  Lastly,wWe  present formulations that demonstrate the impact of different assumptions of channel behavior on sediment transport capacity.
  The implications are discussed for sediment discharge from glaciers with different hydro-climatic forcings.
  We also evaluate the conditions under which subglacial sediment transport capacity could co-vary with water discharge.
  These findings can also improve interpretation of sediment discharge records in glacierized catchments.
\end{abstract}
...
Yet, the effects of evolving channels size could impact the sediment transport response. 
...
These formulations extend the relationships presented in \citet{alley1997}, and illustrate the behavior of different channel types.
Findings indicate differences in the relationships amongst water discharge, channel geometry, and water velocity in subglacial and subaerial channels.
...
x    Stream power & $\Omega$ && $\mathrm{ kg \, m\, s^{-3}}$ \\
...
The first numerical experiment aims to compare the variability between the subglacial and subaerial model outputs to a range of channel shapes, friction factors, and water discharge variability.
...
The much greater water flux and thus subglacial channel size in \icesheet{} drives the larger width integrated shear stress variations (Figure~\ref{fig:Qw}\,c and f).
...
\begin{figure}[hbt!]
  ...
  \caption{Response of sediment transport capacity (orange line) and channel size (bottom row) to a water discharge event under different channel conditions.
    ...
    Gyres on bottom panels represent shear stress on channel walls.
...
Here we examine subaerial channels (Figure~\ref{fig:chan_types} c), steady-state R-channels \citep[Figure~\ref{fig:chan_types} d]{rothlisberger1972} and pipe-flow (i.e. R-channels of fixed size that do not adjust their size to discharge conditions; Figure~\ref{fig:chan_types} a).
The sediment transport capacity is scaled with a given water discharge and hydraulic gradient for three different sediment transport capacity formulas: Meyer-Peter M\"uller  \citep[MPM; ][]{meyer1948}, Engelund and Hansen \citep[EH; ][]{engelund1967}, and Bagnold \citep{bagnold1980}; additionally width-integrated shear stress is assessed, as used as proxy for sediment transport throughout the manuscript.
...
Table~\ref{tab:Qs} shows the scaling of the proxy $w\tau$ as well as $Q_s$ of the MPM, EH, and Bagnold sediment transport formulas with respect to $Q$, $\Psi$ or $S$ for the three different regimes and for different $\alpha$.
Remarkably, the Bagnold formula has a negative exponent for $f$ in all but the pipe-flow regime.
The total transport formula EH gives a slightly stronger dependence on all variables due to the larger exponent on $\tau$ of $\frac{5}{2}$ versus  $\frac{3}{2}$  for the others (Equation~\ref{eq:Qs_eq}).
...
The sediment discharge capacity in the steady state R-channel scales very similarly to the subaerial channel case for all relations and virtually identically for the $\alpha=\frac{1}{3}$ case (Table~\ref{tab:eqs1}).
...
While transport-limited states at glaciers are likely rare \citep[e.g.][]{alley1997}, abundant sediment could persist underneath some glaciers \citep[e.g.][]{walter2014,stevens2022,delaney2022}.
As a result, sediment export can be hydrologically controlled and efficient \citep[e.g.][]{swift2005,andresen2024}.
...
In these cases with high water discharge variability, channels can behave more like a pipe of fixed cross-section \citep[e.g.][]{gimbert2016}.
Table~\ref{tab:Qs} shows that sediment transport in pipe-flow scales much more severely with discharge; the exponent on $Q$ being between $3$ and $5$, compared to the other two regimes when that exponent is at most $\frac{7}{5}$.
...
These sediment transport capacities variations are of far higher magnitude than those of subaerial channels \citep{alley1997}, when evolving subglacial channel size is not considered (Figure~\ref{fig:model_outs}).
Note that pipe flow would cause sediment discharge capacity to covary with water discharge (Figure~\ref{fig:chan_types} a).
However, this covariance does not occur with evolving channel size (Figures~\ref{fig:Qw_vari} and \ref{fig:chan_types} b).
...
Furthermore, some observations from subaerial systems show a co-varying relationship between water discharge and sediment transport in time \citep[e.g.][]{schmidt2006,pitlick2021}.
...
Further evaluation is needed to establish the role of highly variability in sediment transport capacity in evaluating hydro-climatic signals from sediment records.
