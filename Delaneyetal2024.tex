...
\begin{abstract}
  ...
  Using numerical models applied to two observed hydrographs, we evaluate the differences in sediment transport capacity variability between subaerial and subglacial channels.
...
Here three kinds of drainage regimes are examined, namely, pipe-flow (i.e. R-channels which do not have time to adjust their size to discharge conditions; Figure~\ref{fig:chan_types} a), subaerial channels (Figure~\ref{fig:chan_types} c), steady-state R-channels \citep[Figure~\ref{fig:chan_types} d]{rothlisberger1972}.
...
The results of the 12 combinations are presented in Table~\ref{tab:Qs} and also in Table~\ref{tab:Qs2} where the fractions in the exponents are approximately given by decimal numbers for ease of comparison.
...
Conversely, the sediment transport proxies $w_g\,\tau_g$ and $w_f\,\tau_f$ used below scale only as $Q^2$ for pipe-flow, whereas the transport scales at least with $Q^3$.
...
The sediment discharge capacity in the steady state R-channel scales very similarly to the subaerial channel case for all relations and virtually identically for the $\alpha=0.5$ case (Table\ref{tab:eqs1}).
