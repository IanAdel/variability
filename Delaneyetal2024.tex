...
Sediment export from glaciers impacts downstream ecosystems and communities and has changed in response to climate warming and glacier retreat.
Sediment export depends in part on the sediment transport capacity, the amount of sediment that could be transported for the hydraulic conditions with abundant sediment availability.
...
However, subglacial channels are pressurized by the ice above, so that the water flows through a subglacial pipe that evolves in size.
...
In subaerial channels, by contrast, sediment transport capacity is less variable, and variations depends solely on water discharge, not the channel size.
...
Both models use the measured discharge to calculate water velocity, shear stress, and width-integrated shear stress, upon which sediment transport both suspended load and bedload depend \cite<Figure \ref{fig:cartoon}; >{shields1936}.
...
\ian{add comment about glacier geometry}
...
Aggregation periods longer than this  are omitted as they might be conflated with seasonal variations in water discharge.
...
While the seasonal variability is comparable, substantial variations in sediment discharge likely persist between the two channel types from the different timing of peak discharge events.
...
Yet, substantial differences in the variability between the model outputs for subglacial and subaerial cases persist in periods of up to $15$ days.
Note again that the timing of peak sediment transport conditions still occur across these periods, but  are thus not captured in aggregating water discharge over longer periods (Figure~\ref{fig:model_outs}).
...
In subglacial R\"othlisberger channels in steady state with respect to water discharge, the width-integrated shear stress response to water discharge is $ w_g\,\tau_{g}\, \propto\,  Q^{\sim\frac{4}{5}}$ (Table\,\ref{tab:eqs1}).
...
Note that in pipe flow conditions, maximum sediment transport would coincide with peak water discharge and a co-varying relationship with water discharge would occur, unlike the case here with evolving channel size (Figure \ref{fig:model_outs} and \ref{fig:Qw_vari}). 
However, similar adjustments in channel shape occur in subaerial channels \cite{phillips2016}, albeit at  timescales longer than hours to days in subglacial channels. 
...
\subsection{Implications for parameterizing  sediment transport in glacierized catchments}
...
However, the role of channel width in the findings here suggests that more thorough parameterization of channel shape and channel width is needed in assessing subglacial sediment transport capacity as compared to evaluating water pressure (Section\,\ref{sect:sub_mode}, \ref{sect:scaling}).

In applying these models to different climate scenarios, water discharge measurements at the sub-hour timescale remain unavailable in many catchments and daily values might be used, for instance.
The variability in subglacial channels approaches that of subaerial ones  over $1-5$ days (Figure~\ref{fig:multi_run}), possibly resulting in parity in variability between subaerial and subglacial channels over these periods.
...
At the same time, water discharge over periods longer than days may omit key processes, such as the maximum sediment transport capacity occurring with increased water discharge, as opposed to the maximum quantity (Figure~\ref{fig:model_outs}).
...
This multitude of processes lies in contrast to processes occurring in most subaerial channels, where transport capacity typically responds to sediment size, channel shape, water discharge, and hydraulic gradient.
...
\item the timing of peak sediment transport capacity below glaciers generally coincides with large rates of increase in water discharge, as opposed to the maximum water discharge in subaerial channels.
  However, representing subglacial sediment transport capacity with either open-channel flow or pipe flow results in coincident variations; 
  \vspace{.1cm}
  
\item the degree of difference in sediment transport capacity variations between the two channel types depends on the water discharge variability in the hydrograph, which varies with catchment characteristics and water discharge evolution;
  \vspace{.1cm}
