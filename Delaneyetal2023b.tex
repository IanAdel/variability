...
The standard deviation of a output variable over the course of the season is used to quantify the variability of thats output.
...
Rank correlation also reduces the impact uncertainties in the sediment transport record form \icesheet{}.
...
\begin{figure}[h]
  ...
  \caption{Relationship between water discharge and velocity, shear stress, and width-integrated shear stress for \alpine{}  (a-c) and \icesheet (d-f). Variables have been normalized to mean values for better comparison amongst the runs.
...
In comparing output variable with suspended sediment discharge data, velocity, shear stress and width-integrated shear stress from the subaerial system rank correlate better with sediment discharge compared to subglacial outputs (Figure~\ref{fig:Qw_vari} g--l).
The rank correlation among the three output variables for the subaerial parameterization remains constant due to the absence of hysteresis in channel shape, as shown above (see Section~\ref{sect:fluv}).
...
Over longer time aggregation periods in some cases, outputs from the subglacial parameterization outperform those from the subaerial on (see Figures S1-S6).
...
First, large variations in sediment transport capacity underneath glaciers may result in sediment mobilization and deposition processes in close spatial and temporal proximity to each other \citep{gimbert2016,perolo2018}.
...
In turn, the parameterization of subglacial channels here contains a feedback that limits sediment transport capacity as the subglacial channel grows with increasing water discharge capacity, effectively reducing water velocity.
...
In large catchments and further downstream of proglacial margins, water discharge is generally less variable than at catchments' headwaters, where glaciers lie \citep[][]{swift2005,riihimaki2005,costa2017,vanas2017}.
...
Smaller variations here would occur as a result of the hydrological forcing, as opposed to slowly evolving subglacial channel size.


\subsection{Records of sediment discharge  from glaciers}

The  outputs from the subaerial system rank correlate higher with sediment discharge in the \alpine and \icesheet systems here.
This increased performance occurs despite  output variables showing the disparate relationship between sediment transport capacity and water discharge in subglacial systems (Figure~\ref{fig:Qw_vari}).
This could be attributed to several reasons.


First, these measurement stations are $0.5$ to $2$\,\unit{km} downstream of the glacier terminus \citep{cowton2012,felix2022}, respectively.
...
Furthermore,  if a transport-limited regime may persists at the glacier margin, then the representation of subaerial transport processes could be represented just a short distance downstream, as sediment transport could be quickly adjusted to these variations.
Indeed, a strong correlation persists between sediment plume size and water discharge in the Kangerlussuaq fjord, ~ $20$\,\unit{km} downstream of the Leverett measurement site \citep{mcgrath2010}, possibly as a result of open channel flow conditions.
However, at other marine-terminating glacier fjords in Greenland, a less consistent relationships persists between water discharge, or melt extend, and sediment plume size \citep{tedstone2012}.


Second, the parameterizations here consider sediment transport capacity, not the availability or routing of sediment below the glacier and through the proglacial area \citep{delaney2023}.
...
As a result of sediment availability, the sediment transport records presented here may represent a supply-limited sediment transport, less dependent on sediment transport capacity.
...
%Performance metrics of the subglacial width-integrated shear stress and the subaerial metrics are similar (Figures~\ref{fig:Qw_vari}\, i, l and S1-S6).
%Yet, further analysis would be needed to understand the roles of water discharge representing sediment availability, as opposed to  width-integrated shear stress, in this correlation.

Regardless of the cause of the increased performance of water discharge compared to subglacial sediment transport capacity in the observations, parameterization outputs suggest that additional processes and erosional hiatuses may further complicate signals of sediment discharge from glaciers in response to climate, compared to subaerial systems \citep{jansson2005,ganti2016}.
...
Conversely, sediment transport in most subaerial systems typically increases with water discharge and hydraulic gradient, and the hydraulic gradient of most subaerial systems likely remains stable over time scales ranging from years to millenia \citep[Section~\ref{sect:fluv}; e.g.][]{muller1968,whipple1999,wong2006,wickert2019}. 
...
The parameterizations of these processes show substantial differences in subglacial and subaerial channels' response to changing water discharge.
However, subaerial hydraulics demonstrate a better representation of observed sediment discharge variations.
...
\begin{enumerate}
\item the inconsistent relationship between water discharge and sediment transport capacity underneath glaciers, causing increased variability in sediment transport capacity compared to river systems;
\item the timing of peak sediment transport capacity below glaciers generally coincides with large rates of change in water discharge, as opposed to the maximum quantity of water discharge;
\item the possibility for water discharge to represent processes such as subglacial sediment transport capacity, subglacial sediment access, or open channel processes near the glacier margin. 
\end{enumerate}

\section*{Acknowledgments}
\noindent
SNSF Project No. PZ00P2\_202024 provided  funding for I. Delaney.
Code, with links to data, can be found at \url{https://bitbucket.org/IanDelaney/xsection/src/master/}.
