...
On shorter time periods, pressurized water transports sediment from underneath glaciers \cite{walder1994,creyts2013,beaud2018,delaney2019}, should enough sediment be present subglacially.
...
Sediment transport capacity depends on the shear stress between water and the sediment it flows  over \cite{shields1936,meyer1948,engelund1967} and the width of the channel bottom $w_c$ over which to mobilize sediment.
...
The evolution of subglacial channel size $S_g$ is then given as
\begin{linenomath*}
  \begin{equation}
    \label{eq:dS_dt}
  ...
  \end{equation}
\end{linenomath*}
\noindent where $C_1= (1-\rho_wc_pc_t)\,\frac{\rho_wg}{\rho_iL}$ and $C_2=2A(\frac{\rho_wg}{n})^n$ are constants (individual quantities defined in Table \ref{table:vpm}), $l$ is the length of the subglacial channel, $Q$ is water discharge, $\Delta h$ is the hydraulic potential change from the glacier terminus to its top, $h_{o}= \frac{\rho_i}{\rho_w} h_{ice}$ is the ice overburden pressure, and $n$ is the exponent in Glen's law for ice flow \cite{glen1955} (usually $n=3$).
...
The width-integrated shear stress can then be represented by
\begin{equation}
  \label{eq:tautg}
  \tau_{tg}=w_{cg}\,\tau_g,
\end{equation}
...
\end{equation}
%
As above, the width-integrated shear stress is then
\begin{equation}
    \label{eq:tautf}
    \tau_{tf}=w_{cf}\,\tau_f.
  \end{equation}
...
For each glacier, the parameterizations are first applied to a reference test case that assumes  subglacial channel with $\beta=\frac{\pi}{2}$. In this case the friction factors $f_r$ and $f_p$ are tuned so that the parameterizations reproduce reasonable velocities for both  the Greenland Ice Sheet and the Alpine glacier \cite<$\sim\,1.6\,$\unit{m}$^3$\,\unit{s}$^{-1}$>{werder2010b,chandler2013}.
...
For any given variable, the standard deviation of that variable over the course of the season is used to quantify its variability.
...
Outputs and observations, which have a native time resolution of $15$\,\unit{min}, are aggregated across timescales from $1$\,\unit{hr} to $15$\,\unit{days} by averaging values one half of the timescale before and after a given point time.
...
In turn, the subglacial channels contain feedbacks that limit sediment transport capacity as the subglacial channel grows with increasing water discharge capacity. This effectively reduces sediment transport capacity with the stabilization of water discharge.
...
Second, the sediment discharge records could represent processes such as increased sediment accessibility when melt extends up glacier, thereby  increasing sediment and water discharge \cite<e.g.>{vergara2022}.
...
This makes the response of their sediment transport capacity much more sensitive to changes in water discharge when compared to subaerial channels. 
