...
\begin{figure}[h]
  \centering
    ...
    \caption{Variance (measured by the standard deviation) in water velocity, shear stress, and total shear stress for different time aggregations across a large number of subglacial and subaerial channel shapes and friction factors The subglacial (blue) and subaerial (orange) systems are distinguished as are \alpine{} (a-c) and \icesheet{} (d-f).
      ...
      Text in insets shows rank correlation between the variable and sediment discharge for the corresponding time period for both the subglacial (right column) and subaerial (left column) systems.
      \ian{problem with $\tau_g$}
    } 
    \label{fig:multi_run}
  \end{figure}
...
This lies in contrast to other marine-terminating glacier fjords in Greenland that could represent the subglacial signal where less consistent relationships persists between water discharge, or melt extend, and sediment plume size \citep{tedstone2012}.
...
Findings here potentially impacting the interpretation of sediment transport records include:
\begin{enumerate}
