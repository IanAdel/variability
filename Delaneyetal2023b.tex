...
\title{Natural variations in water discharge quantity has an inconsistent impact on sediment transport capacity in subglacial channels}
...
Two gaps (14.07.2012 to 15.07.2012 and 16.07.2012 to 17.07.2021) in suspended sediment data interpolated using a linear spline.
...
For each glacier, the parameterizations are first applied to a reference test case for each glacier that assumes  subglacial channel with $\beta=\frac{\pi}{2}$.
...
Variability of an output is represented by the  standard deviation of an output over the course of the season. 

Lastly, outputs are compared with the sediment discharge records of the glaciers using Spearman rank correlation (i.e. correlation depends on the ordering of values from the outputs and the observations).
...
Outputs and observations at $15$\,\unit{min} intervals are aggregated across timescales of up to $15$\,\unit{days} by averaging values one half of the timescale before and after a given point time.
...
\begin{figure}[h]
  ...
  \caption{Parameterization outputs resulting from the hydrographs shown in panels a and e for \alpine cases (a-d) and \icesheet{} (e-h).
    Gray (black) lines in a and e represent the subglacial channel size (water discharge).
    In other panels, blue lines represent outputs from the subglacial channel, while orange lines represent the subaerial channel.
    Data are shown at $15$\unit{min} intervals.
    Arrows denote some events where  variables peak in the subglacial channel prior to the subaerial one.
...
Over the course of the season, large peaks in water velocity and shear stress, along with width-integrated shear stress, occur in response to water discharge variations in both \alpine{} and \icesheet{}.
...
In the subglacial channel, however, the persistence of high water velocity, shear stress, and width-integrated shear stress across a range of discharges suggests that substantial sediment transport may occur across much of the season (Figure~\ref{fig:Qw_vari} a--f).
...
\begin{figure}[h]
  \centering
    ...
    \caption{Variability (standard deviation) in water velocity, shear stress, and width integrated shear stress for different time aggregations across the range of subglacial and subaerial channel shapes and friction factors.
      Shaded areas denote the minimum and maximum of the $100$ parameter combinations tested (Section~\ref{sect:imp}).
...
The second application examines the impact of a variety of channel shapes and friction factors on the relationship between water discharge and sediment transport capacity.
...
Sediment transport capacity in subglacial channels rapidly responds to changes in water discharge compared to their subaerial counter parts \cite{alley1997}.
...
Appendix~\ref{sect:scaling} provides a complete description of sediment transport capacity in different drainage conditions.

The strong correlation between hydraulics and water discharge in \icesheet{} could result from being closer to an equilibrium between variations in the channels' wetter area and water discharge with smaller diurnal variations in amplitude (Figures~\ref{fig:model_outs} and ~\ref{fig:Qw_vari}).
...
Actually, the transition of subglacial to subaerial conditions may occur underneath glacier terminus \cite{perolo2018}, especially if closure rates of the subglacial channels are slow  \cite{rothlisberger1972}.
...
\begin{tabular}{llllll}
  ...
  \makecell{Equation~\ref{eq:DW}} & Additional relations & Shear stress & Stream p.\\
