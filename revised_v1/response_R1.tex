...
\item  \textbf{Here, most of what is novel seems to fgall under Objective
  ...
  \textit{We value this comment greatly. Upon consideration, we have chosen to rephrase  objective one as ``to establish the hydrological conditions where sediment transport capacity co-varies
    with water discharge in subglacial systems.'' 
    In our view this objective focus the attention on the differences between the ALPINE and ICESHEET scenarios. It also is practical when evaluating the differences in smooth of water discharge, as discussed in the next point.
    We appreciate the need to remove interpretation from the results section. We have done this. Additionally, Results Section 4.3 with the algebraic formulations has been moved to the discussion, as we think the pipe flow and steady state R-channel scenarios serve as useful end members through which to discuss the results.
  }
...
\item \textbf{There are some very important points in Results, which I would attach some key data to
  ...
  \textit{This is an excellent comment. We have worked to address this need for a clearer hysteresis explanation.
    One challenge with the approach recommended by the reviewer is that sediment transport capacity scales very non-linearly with water discharge, especially in subglacial systems. This is shown in Section 4.3.
    As a result, findings across hydrographs would not really be comparable even if they were scaled with a discharge quantity.
    Instead, we will add several paragraphs that to section 4.1 that discuss The role of water discharge variability on hysteresis.
