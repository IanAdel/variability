...
\begin{abstract}
  ...
  While in subglacial channels, water discharge changes primarily affect water velocity due to evolving channel geometry over hours to weeks.
  Hydrographs from an Alpine glacier and the Greenland Ice Sheet are used to drive models of subglacial and subaerial hydraulics, revealing greater variability in the inferred sediment transport capacity in subglacial than in subaerial channels.
...
In these catchments, sediment export depends in part on the sediment transport capacity, the amount of sediment that could be transported for the hydraulic conditions given the abundance of sediment.
This capacity largely depends on the width of the channel and the water's velocity. 
Unlike subaerial channels, like rivers, where the size of the river changes rapidly with water discharge, however, subglacial channels are pressurized by the ice above.
This pressurized flow means that channel size evolves slowly compared to the variations in water discharge, therefore variations in water discharge are largely accommodated by variations in velocity as opposed to channel size.
...
In subaerial channels, by contrast, sediment transport capacity is less variable and depends solely on water discharge.
The channel's different responses to water discharge may impact the interpretation of sediment transport records and respond to the type of hydrological forcing applied.
...
Therefore, on short time scales, subglacial water flow behaves more like pipe flow and changes in water discharge $Q$ are mainly accommodated with increased or decreased water velocity $v$ \cite{alley1997} because the size of the channel $S$ changes on a much longer timescale (Equation~\ref{eq:v} and Figure~\ref{fig:cartoon}).
  ...
  \centering
    ...
    \caption{Sketch for the different short time-scale responses of subglacial and subaerial channels to increased water discharge.
      ...
      The subglacial channel width $w_g$ remain unchanged, while the subaerial channel width $w_f$ evolves. Shear stress $\tau$ is responsible for the mobilization of sediment.
    }
    \label{fig:cartoon}
  \end{figure}
...
Water discharge data is used from Alpine and ice sheet settings collected downstream of the glacier, assuming that no water storage occurred in the proglacial area.
...
Both models use the measured discharge to calculate water velocity, shear stress, and width-integrated shear stress, upon which sediment transport depends \cite<Figure \ref{fig:cartoon}; >{shields1936}.
...
The variability of one model output (i.e water velocity), a key difference between the subaerial and subglacial systems, is represented by its standard deviation throughout the season.
...
Additionally, to be accepted, model runs can only experience a flotation fraction of above $1.2$ for $2.5$\,\% of the run.
...
Aggregation periods longer than this might be conflated with seasonal variations in water discharge and are thus omitted.
In this case, the water discharge input at a given time is the average over the aggregation period before that time.  
...
Examining the relationship between water discharge and the model outputs, different relationship emerges between the subaerial and subglacial cases.
...
\begin{figure}[h]
  \centering
    ...
    \caption{Variability (standard deviation) of model outputs for different time aggregations across the range of subglacial and subaerial channel shapes and friction factors.
      Shaded areas denote the range of results from the accepted $100$ parameter combinations tested (Section~\ref{sect:imp}).
...
Across the range of channel shape parameters and friction factors  examined (Section\,\ref{sect:imp}), variability in water velocity and shear stress remains higher in the subglacial system compared to the subaerial one.
...
Together these findings suggest that sediment transport capacity variations over a melt season can approach in subaerial and subglacial systems if substantial variability persists in the hydrological forcing. 
\ian{state that these are generally higher, is t-test needed? }

Evaluating the different variability in sediment transport characteristics across timescales can help inform the sediment transport processes omitted from applying daily or weekly values of water discharge to understand sediment transport.
...
Yet, substantial differences in the variability of model outputs for subglacial and subaerial cases persist in periods of up to $15$ days.
Note that lags and time of peak sediment transport conditions still occur over these periods, and these characteristics are thus not considered in aggregating outputs and water discharge over longer periods (Figure~\ref{fig:model_outs}).
...
In R-channel models, viable water pressure regimes can be achieved by changing the shape and friction factor of the channels, enabling assumptions of semi-circular channels in most applications \cite{werder2010b}.
...
\ian{Similar feedback occurs in subaerial channels \cite{phillips2016}, albeit at  timescales longer than days in subglacial channels. } 
...
In subglacial R\"othlisberger channels in steady state with respect to water discharge, the width-integrated shear stress response to water discharge is $ w\tau_{tg}\, \propto\,  Q^{\sim\frac{4}{5}}$ (Table\,\ref{tab:eqs1}).
However, subglacial channels rarely operate in a steady state \cite<e.g.>{gimbert2016}.
For sufficiently short periods, channel width and wetted area are fixed in the subglacial system (akin to pipe flow) so that the response of width-integrated shear stress is $ \tau_{tg}\, \propto\,  Q^{2}$ (Table\,\ref{tab:eqs1}).
\ref{sect:scaling} provides a complete description of sediment transport capacity in different drainage conditions.

The co-varying relationship between sediment discharge capacity in  subaerial conditions could be pronounced $\sim20$\,\unit{km} downstream of the Leverett site, where a strong correlation persists between sediment plume size and the river's water discharge into the Kangerlussuaq fjord \cite{mcgrath2010}.
...
Our results point to the highly variable response of proxies for sediment transport capacity to  water discharge quantity, yet substantial differences emerge between the \alpine{} and \icesheet{} experiments.
Indeed, the more independent relationship between model outputs and water discharge in \alpine{} is smaller compared to \icesheet{} due to the relatively larger discharge variations on daily to weekly timescales.
...
In this case, the exponent on water discharge for shear stress is likely significantly less than $2$, but greater than $\frac{4}{5}$ that occurs when the R-channel's size evolves in equilibrium with water discharge (see \ref{sect:scaling}).
As a result, a stronger correlation persists between subglacial model outputs and water discharge in \icesheet{} compared to \alpine{} (Figure~\ref{fig:Qw_vari}\,a-f).
...
If water input before these events have been minimal, the channel size would be small and sediment transport capacity would very rapidly increase at the onset of the event (see \ref{sect:scaling}).
...
Water discharge measurements at the sub-hour timescale remain unavailable in many catchments and daily values might be used, for instance.
The variability in subglacial channels approaches that of subaerial ones  over $1-5$ days (Figure~\ref{fig:multi_run}), possibly resulting in parity between subaerial and subglacial channels over these periods.
This suggests that aggregating water discharge over daily to weekly periods could yield similar behavior between subglacial and subaerial systems. 
At the same time, water discharge over such periods may omit key processes, such as water velocity reaching sediments' threshold of motion more frequently or the timing of peak events.
...
\item the degree of difference in sediment transport capacity variations between the two channel types depends on the water discharge variability in the hydrograph, which varies with geography and across timescales.
  ...
  \item the uneven response of subglacial sediment transport capacity to water discharge variations shows a different response to periods of elevated water discharge in subglacial channels compared to subaerial ones.
\end{enumerate}
