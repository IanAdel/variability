...
Over long periods, such as millenia, processes such as glacier abrasion and quarrying sculpt landscapes and create sediment to be transported fluvially from under glaciers \cite<c.f.>{hallet1979,iverson2012,ugelvig2018}.
...
The water discharge data used here was collected at a $1$\unit{min} interval from May 24, 2014 to October 10, 2014 \citeA{felix2021,felix2022}.
...
To do this, we use a lumped hydraulics model from \citeA{werder2010b}, itself based upon  \cite{clarke1996}.
...
In the first experiment, for each glacier, the models are  applied to a reference test case  that assumes  a subglacial channel with $\beta=\frac{\pi}{6}$%, i.e. a segment of a quarter-circle,
(Table~\ref{table:vpm}).
Both friction factors $f_r$ and $f_f$ are tuned so that reasonable water velocities \cite<$\sim\,1.6\,$\unit{m}$^3$\,\unit{s}$^{-1}$>{werder2010b,chandler2013} occur for both \alpine{} and \icesheet{}.
...
Model outputs are compared to water discharge from the glaciers using Spearman rank correlation, which varies with the ordering of values.
...
The effect of channel shape and roughness is established by running the model with random parameter values of channel geometry factors ($\beta$ and $k$) and friction factors ($f_r$ and $f_f$; see Table~\ref{table:vpm} for values).
...
Similar adjustments in channel shape occur in subaerial channels \cite{phillips2016}, albeit at timescales longer than the subglacial channel's response of hours to days.
...
Here, ocean water pressurizes subglacial channels at the ice front \cite<e.g.>{how2017}, so the reduced correlation could result from the inconsistent relationship between subglacial sediment transport capacity and water discharge.
...
In subglacial systems, sediment's critical shear stress, or threshold at which sediment mobilization begins, can be reached more frequently and across a range of water discharges, compared to subaerial systems (Figure~\ref{fig:Qw_vari}).
